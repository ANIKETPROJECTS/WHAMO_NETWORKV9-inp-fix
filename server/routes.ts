...
export async function registerRoutes(
  ...
  app.post("/api/run-whamo", (req, res) => {
    ...
    if (fs.existsSync(exePath)) {
      execFile("wine", [exePath, "network.inp", "network.out"], { cwd: tempDir }, (error, stdout, stderr) => {
        if (error) {
          console.error("WHAMO Execution Error:", error);
          const generatedOutPath = path.join(tempDir, "network.out");
          if (fs.existsSync(generatedOutPath)) {
             return res.download(generatedOutPath, "network.out");
          }
        ...
        res.download(generatedOutPath, "network.out");
      });
    } else {
